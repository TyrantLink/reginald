...
class birthdays(commands.Cog):
	...
	async def birthday_setup(self,ctx:SlashContext,role:discord.Role,channel:discord.TextChannel):
		if not data.read(['servers',str(ctx.guild.id),'config','enableBirthdays']): await ctx.send(embed=discord.Embed(title='Birthdays is not enabled on this server.',color=0x69ff69)); return
		data.write(role.id,['servers',str(ctx.guild.id),'birthdayRole'])
		data.write(channel.id,['servers',str(ctx.guild.id),'birthdayChannel'])
	...
	async def birthday_set(self,ctx:SlashContext,month:int,day:int):
		...
		data.write(f'{month.zfill(2)}/{day.zfill(2)}',['users',str(ctx.author.id),'birthday'])
		await ctx.send(f"birthday successfully set to {data.read(['users',str(ctx.author.id),'birthday'])}")
		logOutput(f"birthday set to {data.read(['users',str(ctx.author.id),'birthday'])}",ctx)
	...
	async def birthday_remove(self,ctx:SlashContext):
		data.write(None,['users',str(ctx.author.id),'birthday'])
