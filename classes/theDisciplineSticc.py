...
class theDisciplineSticc(commands.Cog):
	...
	async def setup(self,ctx:SlashContext,role:discord.Role,channel:discord.TextChannel):
		if not data.read(['servers',str(ctx.guild.id),'config','enableTalkingStick']): await ctx.send(embed=discord.Embed(title='The Talking Stick is not enabled on this server.',color=0x69ff69)); return
		data.write(role.id,['servers',str(ctx.guild.id),'tsRole'])
		data.write(channel.id,['servers',str(ctx.guild.id),'tsChannel'])
	...
	async def leaderboard_sticcs(self,ctx:SlashContext):
		if not data.read(['servers',str(ctx.guild.id),'config','enableTalkingStick']): await ctx.send(embed=discord.Embed(title='The Talking Stick is not enabled on this server.',color=0x69ff69)); return
		data.write({key: value for key, value in sorted(data.read(['servers',str(ctx.guild.id),'tsLeaderboard']).items(), key=lambda item: item[1],reverse=True)},['servers',str(ctx.guild.id),'tsLeaderboard'])
		names = []
		index = 1
		await ctx.defer()
		for member in data.read(['servers',str(ctx.guild.id),'tsLeaderboard']):
			if member not in data.read(['variables','idNameCache']): data.write((await client.fetch_user(member)).name,['variables','idNameCache',member])
			rank = str(index)+("th" if 4<=index%100<=20 else {1:"st",2:"nd",3:"rd"}.get(index%10, "th")); index += 1
			names.append(f"{rank} - {data.read(['variables','idNameCache',member])}: {data.read(['servers',str(ctx.guild.id),'tsLeaderboard',member])}")
	...
	async def list_active(self,ctx:SlashContext):
		...
		active = []
		await ctx.defer()
		for member in data.read(['servers',str(ctx.guild.id),'activeMembers']):
			if member not in data.read(['variables','idNameCache']): data.write((await client.fetch_user(member)).name,['variables','idNameCache',member])
			active.append(data.read(['variables','idNameCache',member]))
